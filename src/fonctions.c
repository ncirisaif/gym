...
int verifiery( char nom[], char pwd[])
{ 
int a;
FILE* f;
char ch[30],ch1[30];
...
void ajoutery(char nom[],char prenom[],char adresse[] ,char tel[],char cin[],char rib[],int jour , int mois , int annee)
{FILE*f;

f=fopen("myspacey.txt","w");
...
void affichery(GtkWidget *pListView)
{
enum {
NOM,
...
void ajouter_horairey(Datee d)
{	FILE* f;
	f=fopen("horairey.txt","a+");
  	if(f!=NULL) 
 		{  fprintf(f," %d %d %d %s \n", d.jour ,d.mois, d.annee, d.temps);  	
	fclose(f);  } 	
...
void afficher_horairey(GtkWidget *pListView)
{
enum {
JOUR,
...
void ajouter_rdvy(char nom1[],char prenom1[],Date1 d1)
{	FILE* f;
...
void afficher_rdvy(GtkWidget *pListView)
{
enum {
NOM1,
...
int verifier_rendy(Date1 sa)
{
Date1 s;
int v=0; // salle non reserve
FILE *f5;
char ch1[50] ; char ch2[50];
f5=fopen("RDV_KINE.txt","r");
if(f5!=NULL)
{
while(fscanf(f5,"%s %s %d %d %d %s ",ch1,ch2,&s.jour1,&s.mois1,&s.annee1,s.temps1)!=EOF)
{if (s.jour1==sa.jour1) {
...
int verifier_horairey(Datee sa)
{
Datee s;
int v=0; // salle non reserve
FILE *f6;
f6=fopen("horairey.txt","r");
if(f6!=NULL)
{
